...
- **効果音とミュート / Sound & mute** — 効果音は Web Audio API で合成しているので音声ファイルは不要（1ファイル完結のまま）。鳴るのは「プレー開始」「茶箱の着弾」「終了（クリア／時間切れ）」の場面で、コンボ量による音の変化はなし。ただし **👑御用船への直撃だけは専用の派手な音**（上昇する和音＋ベル）が鳴ります。また、潮（TIDE）が**明るい帯にいる間は、タイミングの合図として柔らかい波音がそっと寄せて引きます**（耳で「今が×2」が分かる補助）。画面右上の **🔊 / 🔇** ボタンでミュート切替できます。ブラウザの自動再生制限のため、音はスタートボタンを押した瞬間から有効になります。
  Sound effects are synthesized with the Web Audio API — no audio files needed (still one file). They play on **start**, on each **crate landing**, and at the **end** (clear / time-up); no variation by combo size. A flashier sound plays only on a **👑 royal-cargo direct hit**, and a **soft wave wash swells while the tide marker is in the bright band** as an audible cue for the ×2 timing. Toggle mute with the **🔊 / 🔇** button at the top right. Per browser autoplay rules, audio activates the moment you press start.
